...
int js_atime_create(int limit_count){
   ...
   */
   ...
   sprintf(buffer,"SELECT PATH, time from USB where time > %s ORDER BY time LIMIT %d",newValues[3],LIMITCOUNT);
 ...
 /*  sprintf(buffer,"SELECT PATH,TIME, TYPE from Registry where TIME < %s ORDER BY TIME DESC LIMIT %d",newValues[3],LIMITCOUNT);
   ...
   */
   ...
   sprintf(buffer,"SELECT PATH, time from USB where time < %s ORDER BY time DESC LIMIT %d",newValues[3],LIMITCOUNT);
