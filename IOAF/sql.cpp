...
int reg2sql(REGQUEUE *q)
{
	...
	{
		puts("Insert Query prepare 에 실패하였습니다.");
	}
	...
	while(! q->empty()){
		...
		while (!q->empty() && c<wait_size)
		{
			REGISTRY *tmp;
			tmp=q->front();
			q->pop();
			sqlite3_bind_text(stmt, 1, tmp->key, strlen(tmp->key), SQLITE_STATIC);
			sqlite3_bind_int(stmt, 2, tmp->time);
		...
		printf("%ld/%d개 넣음\n", (long)unit_count*(long)wait_size, qs);
