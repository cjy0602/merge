...
int js_ctime_create(int limit_count){
   ...
   */
   ...
   sprintf(buffer,"SELECT PATH, time from USB where time > %s ORDER BY time LIMIT %d",newValues_ctime[3],LIMITCOUNT_ctime);
   ...
   */
   ...
   sprintf(buffer,"SELECT PATH, time from USB where time < %s ORDER BY time DESC LIMIT %d",newValues_ctime[3],LIMITCOUNT_ctime);
